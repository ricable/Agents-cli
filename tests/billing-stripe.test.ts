...
describe("StripeProvider — mock mode (no API key)", () => {
  ...
  beforeEach(() => { savedKey = process.env["STRIPE_SECRET_KEY"]; delete process.env["STRIPE_SECRET_KEY"]; });
  afterEach(() => { if (savedKey !== undefined) process.env["STRIPE_SECRET_KEY"] = savedKey; else delete process.env["STRIPE_SECRET_KEY"]; });
